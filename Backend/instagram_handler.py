...
def extract_instagram_policy():
    ...
    try:
        ...
        if response.status_code == 200:
                ...
                if script.string and 'privacy' in script.string.lower():
                    ...
                    try:
                        ...
                        json_match = re.search(
                            r'\{.*"privacy".*\}', script.string, re.DOTALL)
                        if json_match:
            ...
            for line in lines:
                keywords = [
                    'privacy', 'policy', 'data', 'information', 'collect',
                    'use', 'share', 'personal', 'user', 'account', 'profile',
                    'content', 'photo', 'video', 'message', 'communication',
                    'location', 'device', 'browser', 'cookie', 'tracking',
                    'analytics', 'advertising', 'third-party', 'legal',
                    'rights', 'access', 'delete', 'modify', 'retention',
                    'security', 'encryption'
                ]
                if (len(line) > 20 or
...
def get_instagram_policy_summary():
    ...
    # Since we can't easily extract the full policy, provide summary
